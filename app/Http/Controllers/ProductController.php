...
class ProductController extends Controller
{
	...
	{	
		...
		return view('users.product')->with('users', $users)->with('item_v',$item);
	}
	...
	{	
		...
		$adver = Product::find($id);
		if( $adver == null )
		{
			return redirect()->intended('admin/product?page='.$page);
		}
									
		return view('users.product_register')->with('adver',$adver)->with('error', '')->with('mode', 'edit');
    ...
    {	
		...
		$adver->title = $data['title'];
		$adver->thumbpath = $data['thumb_url'];
		$adver->description = $data['desc'];
		$adver->start = strtotime($data['start']);
		$adver->end = strtotime($data['end']);
		$adver->sequence = $data['sequence'];
		$adver->save();
		
		return view('users.product_register')->with('adver',$adver)->with('error', 'SUCCESS')->with('mode', 'edit');
	}		
