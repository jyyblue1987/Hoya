...
(function($){
	...
	Zoomple.prototype.init = function(){
		...
		if(pageflag==1)
		    if(winwidth >= backimgwidth)
		    { 
			    //document.getElementById("simulator-backimg").style.width="100%";
			    //document.getElementById("simulator-backimg").style.height=backimgheight;
	...
	Zoomple.prototype.showZoom = function($img,e ){
		...
		switch(options.pageNum){
			...
			//case 4://=====product progressive page=====
			       //this.delaier($img.parent().attr(options.source),$img.attr('alt'));
				  // break;
		    case 5://=====product photocromics page=====
			       this.delaier($img.parent().attr(options.source),$img.attr('alt'));
				   break;
		}
	...
	Zoomple.prototype.moveZoom = function($target,e){
		...
		switch(this.options.pageNum){
			...
			break;
		  	/*case 4://====product progressive page===
		           var divOffset = document.getElementById('pop_progress_reg').getBoundingClientRect();
		           var divId = document.getElementById('pop_progress_reg');
			       var divId_width = parseInt(divId.style.width);
			       if(!e) var e = window.event;
		           var x = e.clientX ;
		           var y = e.clientY ;
			       //alert(x + ', ' + y+','+divOffset.top+','+divOffset.left+','+divId_width);
		           if(x > divOffset.left && x<(divOffset.left+divId_width)&&y>divOffset.top && y<(divOffset.top+50))
                      this.hideZoom();
			break;  */
			case 5://====product AR page===
	...
	{	
		...
		switch(this.options.pageNum){
		     ...
		     case 3:this.timer = setTimeout($.proxy(this.delaiedZoom,this,imgARURL,imgDescription), this.options.delay); break;
	...
	Zoomple.prototype.setImageOverlay = function(){
		...
		{
			//document.getElementById("simulator-backimg").style.width="100%";
			//document.getElementById("simulator-backimg").style.height="100%";
		}
